...
RUN cd backend \
    ...
    && .venv-simulation/bin/python -m ensurepip \
    && .venv-simulation/bin/pip install --no-cache-dir \
...
RUN apt-get update \
  ...
  && apt-get install -y --no-install-recommends nodejs \
  && rm -rf /var/lib/apt/lists/* \
  && useradd -m -u 1000 mirofish

# Copy uv from builder (needed by npm run backend -> uv run)
COPY --from=ghcr.io/astral-sh/uv:0.9.26 /uv /uvx /bin/
...
# App source (changes most frequently — last layer)
COPY . .
