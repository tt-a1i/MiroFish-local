...
class OasisProfileGenerator:
    ...
    Converts entities from Zep knowledge graph into Agent Profiles required by OASIS simulation

    Optimized features:
    1. Calls Zep knowledge graph retrieval for richer context
    ...
    # Group/organization entity types (require representative persona generation)
    GROUP_ENTITY_TYPES = [
    ...
    def __init__(
        ...
        # Zep client for retrieval of rich context (uses adapter factory)
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep knowledge graph hybrid search to get rich information related to the entity

        Zep does not have a built-in hybrid search interface, so edges and nodes are searched separately and results merged.
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            # 3. Handle newlines within strings
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for public opinion simulation, faithfully reflecting existing real-world information. Must return valid JSON format, all string values must not contain unescaped newlines. Use Chinese."
        return base_prompt
    
    def _build_individual_persona_prompt(  # noqa: C901
        self,
        entity_name: str,
        entity_type: str,
        entity_summary: str,
        ...
        return f"""Generate a detailed social media user persona for the entity, faithfully reflecting existing real-world information.
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Stance and viewpoints (attitude toward the topic, what might anger/move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memories (important part of persona; describe this individual's connection to the event and their existing actions/reactions in the event)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an organization/group entity, faithfully reflecting existing real-world information.
...
2. persona: Detailed account profile description (2000 characters of plain text), must include:
   ...
   - Special notes (demographics of represented group, operational habits)
   - Organizational memory (important part of the persona; describe this organization's connection to the event and its existing actions/reactions in the event)
3. age: Fixed value 30 (virtual age for organizational accounts)
4. gender: Fixed value "other" (organizations use "other" to indicate non-individual)
...
- persona must be a continuous text description, do not use newline characters
- Use Chinese (except gender field must use English "other")
- age must be integer 30, gender must be string "other"
- Organizational account communication must align with its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: Real-time output file path (if provided, writes each profile as generated)
        ...
        completed_count = [0]  # Use list for mutable access in closure
        lock = Lock()
        ...
        logger.info(f"Starting parallel generation of {total} agent personas (parallel_count: {parallel_count})...")
        print(f"\n{'='*60}")
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter profiles in CSV format (per OASIS official requirements)
