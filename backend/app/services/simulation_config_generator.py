...
Step-by-step generation strategy to avoid failures from generating too much at once:
...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        ...
        "morning": 0.4,    # Gradually becoming active
        "work": 0.7,       # Moderate activity during work hours
        "peak": 1.5,       # Evening peak
        "night": 0.5       # Declining activity late night
    }
}
...
class TimeSimulationConfig:
    """Time simulation configuration (based on Chinese daily activity patterns)"""
    # Total simulation duration (in simulation hours)
    total_simulation_hours: int = 72  # Default: simulate 72 hours (3 days)

    # Time represented per round (in simulation minutes) - default 60 minutes (1 hour), accelerated time flow
...
class EventConfig:
    """Event configuration"""
    # Initial events (triggering events at simulation start)
...
class SimulationConfigGenerator:
    ...
    information, automatically generating optimal simulation parameter configurations.
...
- The user group is Chinese; follow Beijing time activity patterns
...
- General pattern: low early morning, gradual morning increase, moderate work hours, evening peak
- **Important**: The example values below are for reference only; adjust specific time periods based on event nature and participant characteristics
  - For example: student group peak may be 21-23; media active all day; official agencies only during work hours
  - For example: breaking news may cause late-night discussion, off_peak_hours can be shortened
...
- **Time follows Chinese activity patterns**: almost no activity 0-5 AM, most active 7-10 PM
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configuration should follow Chinese daily activity patterns."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single agent config based on rules (Chinese activity patterns)"""
