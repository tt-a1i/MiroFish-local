...
def _get_simulation_python() -> str:
    ...
    Priority:
    1. Environment variable SIMULATION_PYTHON
    2. Project directory .venv-simulation/bin/python
    3. Fallback to current Python interpreter

    The simulation environment requires camel-oasis/camel-ai (neo4j==5.23.0),
    which is incompatible with the main environment's graphiti-core (neo4j>=5.26.0),
    so they must run in isolation.
    """
    ...
    # 2. Check for isolated simulation environment in project directory
    ...
    # 3. Fallback to current interpreter (may fail due to dependency conflicts)
    logger.warning("No isolated simulation environment found, using current Python (may have dependency conflicts)")
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation configuration does not exist, please call /prepare API first")
        ...
        try:
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensuring all related processes are terminated when the server shuts down
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (for forcing a fresh simulation start)
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True if environment is alive, False if environment is shut down
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: Interview on both platforms simultaneously in dual-platform simulation, return consolidated result
